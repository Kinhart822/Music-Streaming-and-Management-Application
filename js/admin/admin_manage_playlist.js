document.addEventListener('DOMContentLoaded', () => {
    ...
    const populateEditForm = (index) => {
        ...
        if (!playlist) {
            console.error('Playlist not found at index:', index);
            ...
            ? paginatedPlaylists.map((playlist) => {
                ...
                return `
                    ...
                    : 'None'
                        ...
                        <td>
                            ${
                    playlist.status === 'draft' || playlist.status === 'pending'
                        ? `
        ...
        if (isValid) {
            ...
            try {
                if (editIndex !== null) {
                    ...
                    // Sort playlists to ensure new playlist appears in correct position
                    playlists.sort((a, b) => {
                        if (currentSort === 'title-asc') {
                            return a.title.localeCompare(b.title);
                        } else if (currentSort === 'title-desc') {
                    ...
                    // Find the index of the new playlist after sorting
                    const newPlaylistIndex = playlists.findIndex(p => p.id === playlistData.id);
                    // Calculate the page where the new playlist should appear
                    let filteredPlaylists = [...playlists];
                    if (currentFilterStatus !== 'all') {
                        filteredPlaylists = filteredPlaylists.filter(p => p.status.toLowerCase() === currentFilterStatus);
                    }
                    if (searchQuery) {
                        const query = searchQuery.toLowerCase();
                        filteredPlaylists = filteredPlaylists.filter(p => p.title.toLowerCase().includes(query));
                    }
                    const newPlaylistFilteredIndex = filteredPlaylists.findIndex(p => p.id === playlistData.id);
                    if (newPlaylistFilteredIndex !== -1) {
                        currentPage = Math.floor(newPlaylistFilteredIndex / rowsPerPage) + 1;
                    } else {
                        currentFilterStatus = 'all';
                        searchQuery = '';
                        filterStatusSelect.value = 'all';
                        searchInput.value = '';
                        currentPage = Math.floor(newPlaylistIndex / rowsPerPage) + 1;
                    }
    ...
    playlistTableBody.addEventListener('click', (e) => {
        ...
        if (isNaN(globalIndex) || globalIndex < 0 || globalIndex >= playlists.length) {
        ...
        if (target.matches('.view-songs')) {
            ...
            try {
                playlists[globalIndex].status = 'accepted';
                localStorage.setItem('playlists', JSON.stringify(playlists));
                renderTable();
                alert(`Playlist "${playlists[globalIndex].title}" accepted.`);
            } catch (e) {
                alert('Failed to accept playlist: Storage quota exceeded.');
            }
        } else if (target.matches('.edit')) {
            populateEditForm(globalIndex);
        } else if (target.matches('.decline')) {
            try {
                playlists[globalIndex].status = 'declined';
                localStorage.setItem('playlists', JSON.stringify(playlists));
                renderTable();
                alert(`Playlist "${playlists[globalIndex].title}" declined.`);
            } catch (e) {
                alert('Failed to decline playlist: Storage quota exceeded.');
            }
